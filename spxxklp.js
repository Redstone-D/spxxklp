...
// @author      SPGoding & SPX Fellow & 我是Redstone & Cinder
...
[color=#388e3c][font=-apple-system, BlinkMacSystemFont, &quot;][size=5]|[/size][/font][/color][size=4][font=-apple-system, BlinkMacSystemFont, &quot;]本文内容按照 [/font][/size][font=-apple-system, BlinkMacSystemFont, &quot;][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57]CC BY-NC-SA 4.0[/color][/size][/url][/font][size=4][font=-apple-system, BlinkMacSystemFont, &quot;] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][hr]\n`;
  ...
  Converted at ${time.getFullYear()}-${padTime(time.getMonth() + 1) // why +1 javascript
    ...
    (function (VersionType) {
      ...
      VersionType[VersionType["BedrockRelease"] = 4] = "BedrockRelease";
    ...
    const converters = {
      ...
      dl: async (ele, ctx) => {
        ...
        const ans = `\n\n${await converters.recurse(ele, ctx)}\n【本文排版借助了：[url=https://github.com/SPXFellow/spxx][color=#388d40][u]SPXX[/u][/color][/url] v${spxxVersion}】\n\n`;
        return ans;
      },
      dd: async (ele, ctx) => {
        let ans = '';
    ...
    function getVersionType(url) {
      ...
      } else if (url.toLowerCase().includes('minecraft-bedrock')) {
    ...
    async function convertZendeskArticleToBBCode(html, articleUrl, translator = config.translator, titleSlice, contentClass, versionType) {
      ...
      const header = versionType ? getHeader('news'||'marketplace', versionType) : `[color=#388e3c][font=-apple-system, BlinkMacSystemFont, &quot;][size=5]|[/size][/font][/color][size=4][font=-apple-system, BlinkMacSystemFont, &quot;]本文内容按照 [/font][/size][font=-apple-system, BlinkMacSystemFont, &quot;][url=https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans][size=4][color=#2e8b57]CC BY-NC-SA 4.0[/color][/size][/url][/font][size=4][font=-apple-system, BlinkMacSystemFont, &quot;] 协议进行授权，[b]转载本帖时须注明[color=#ff0000]原作者[/color]以及[color=#ff0000]本帖地址[/color][/b]。[/font][/size][hr]\n`;
      const footer = versionType ? getFooter('news'||'marketplace', versionType) : `[hr][color=#388e3c][size=5]|[/size][/color][size=4][b]想了解更多游戏资讯？[*][/b][/size][size=4][size=3][url=https://klpbbs.com/forum-2-1.html]苦力怕论坛 - 游戏资讯版块[/url][/size][/indent]`;
  ...
  【本文排版借助了：[url=https://github.com/SPXFellow/spxx][color=#388d40][u]SPXX[/u][/color][/url] Userscript v${spxxVersion}】[/indent][/indent]\n\n${footer}`;
      return ans;
    }
